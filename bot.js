...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("Cloud Roleplay Szerver Státusz", "https://cdn.discordapp.com/attachments/724725749323661423/724725850355925042/acloud.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ CloudRP' : '⚫ CloudRP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nInformáció:','**Szerver IP**: 92.42.46.147:30418 **Discord**: https://discord.gg/BecAyvw Amennyiben problémába ütköztél nézz utána a segítségkérés szobában. Ha nem találsz rá megoldást, akkor keress fel egy **Admint!** YouTube csatornánk: **[Cloud RolePlay](https://www.youtube.com/channel/UCETy-Kw3Qbpid7Uu3zCKPZA?)**\n\u200b\n',false)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: A szerver aktuálís állapota:',`${STATUS}\n\u200b\n`);
  ...
  const offline = function() {
    ...
    .setColor(0xff0000)
    .addField('Szerver Státusz',':x: Leállva',true)
    .addField('Sorban állók száma','Nincs',true)
    .addField('Elérhető játékosok','nincs\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addField('Sorban állók száma',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Játékosok',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('CloudRP',{'url':'','type':'In game'});
    ...
    if (!message.author.bot) {
      if (message.member) {
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setColor(0x2894C2)
          .setTitle('Bug Report')
          .setDescription('Sikeresen rögzítettük az adott problémádat, hamarosan utána nézünk!')
          .setTimestamp(new Date());
